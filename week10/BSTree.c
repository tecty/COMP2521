...
{
    // couldn't search for the root node is not exist
    if (t == NULL) {
        return;
    }
    ...
    // initial the queue
    Queue q= newQueue();
    // join the queue with the root
    QueueJoin(q, t);
    while (!QueueIsEmpty(q)) {
        /* search for each node */
        BSTree current_node = QueueLeave(q);
        // print the current node
        showBSTreeNode(current_node);
        // join it's leaves into searching
        if (current_node->left != NULL) {
            /* join into queue */
            QueueJoin(q,current_node->left);
        }
        if (current_node->right != NULL) {
            /* join into queue */
            QueueJoin(q,current_node->right);
        }
    }
